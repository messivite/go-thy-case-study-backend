-- chat_messages soft-delete alanı
alter table public.chat_messages
    add column if not exists deleted_at timestamptz null;

create index if not exists idx_chat_messages_session_not_deleted_created
    on public.chat_messages (session_id, created_at desc)
    where deleted_at is null;

comment on column public.chat_messages.deleted_at is
    'Mesaj soft delete zamanı; null ise aktif.';

-- Search RPC: silinmiş mesajları dışla
...
-- Session list RPC: son mesaj preview'de silinmiş mesajları dışla
...
-- Message page RPC: silinmiş mesajları dışla
